...
def scale_data(processed_data_path):
    ...
    pd.DataFrame(data=X_train_scaled, columns=column_names).to_csv(f"{processed_data_path}/X_train_scaled.csv", index=False)
    pd.DataFrame(data=X_test_scaled, columns=column_names).to_csv(f"{processed_data_path}/X_test_scaled.csv", index=False)
